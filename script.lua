...
-- Buy selected gear or all gear if "All Gear" is selected
local function BuySelectedGear()
    local Gear = SelectedGear.Selected
    if not Gear or Gear == "" then return end

    if Gear == "All Gear" then
        for G, Stock in pairs(GearStock) do
            if G ~= "All Gear" and Stock > 0 then
                for i = 1, Stock do
                    BuyGear(G)
                    wait(0.1)
                end
        ...
        local Stock = GearStock[Gear] or 0
        if Stock > 0 then
            for i = 1, Stock do
                BuyGear(Gear)
                wait(0.1)
...
SelectedGear = GearNode:Combo({
    ...
    Callback = function(Value)
        if AutoGear then
            AutoGear:SetLabel(Value == "All Gear" and "Auto-Buy All Gear" or "Auto-Buy Selected Gear")
        end
    end
})
...
-- Auto-buy loop (skips gear with 0 stock)
coroutine.wrap(function()
    while wait(0.5) do
        if AutoGear.Value then
            GetGearStock()      -- refresh gear list
            BuySelectedGear()   -- buy gear or all gear if selected
        end
    end
end)()
